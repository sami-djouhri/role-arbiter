...
# Terraria mit Mod-Weg: der Server bleibt 'manuell' (tModLoader kommt nicht ueber Steam),
# seine Workshop-Mods haben trotzdem einen automatischen Weg.
REGISTRY_MODS = {"games": [
    {"name": "terraria", "service": "terraria-server",
     "update": {"kind": "manuell", "hinweis": "tModLoader von Hand",
                "mods": {"kind": "workshop", "appid": 1281930, "user": "terraria",
                         "steamcmd": "/home/terraria/steamcmd/steamcmd.sh",
                         "install_dir": "/home/terraria",
                         "liste": "/home/terraria/Mods/install.txt",
                         "acf": "/home/terraria/steamapps/workshop/appworkshop_1281930.acf"}}},
    {"name": "zomboid", "service": "zomboid-server",
     "update": {"kind": "steam", "appid": 380870, "anonym": True, "user": "zomboid",
                "steamcmd": "/s", "dir": "/d"}},
]}
...
class ModWeg(unittest.TestCase):
    """Die zweite Arbeit im selben Ablauf. Terraria ist der Fall, der sie erzwungen hat:
    tModLoader zieht seine Mods NICHT beim Serverstart nach, sondern nimmt den lokalen
    Workshop-Cache. Gemessen am 2026-09-19 lag Recipe Browser 292 Tage zurueck, waehrend
    der Server selbst lief und bespielt wurde."""

    def setUp(self):
        self.tmp = tempfile.mkdtemp()
        self.m = load(self.tmp, REGISTRY_MODS)
        self.a = Attrappe(self.m)
        self.geladen = []
        def update_mods(mods, trocken):
            self.a.schritte.append(("plan:" if trocken else "") + "workshop:%d" % mods["appid"])
            self.geladen.append(mods)
            return self.a.update_ok, "3 von 3 Mod(s) geladen oder bestaetigt"
        self.m.update_workshop_mods = update_mods
        self.m.mod_stand = lambda mods: {"2619954303": 1700000000}

    def test_mods_bekommen_dieselben_schutzschritte(self):
        """Der Grund, warum das keine zweite Funktion ist: ein Mod-Update ohne Wartung
        ...
        self.assertLess(s.index("--wartung-aus terraria"), s.index("--wake terraria --live"),
                        "sonst lehnt die eigene Sperre den eigenen Probestart ab")

    def test_spieler_online_bricht_auch_beim_mod_update_ab(self):
        self.a.spieler = 3
        self.a.laeuft = True
        self.assertEqual(self.m.aktualisieren("terraria", live=True, mods=True), 4)
        self.assertEqual(self.a.schritte, [])

    def test_mod_weg_wird_nicht_mit_dem_server_weg_verwechselt(self):
        """Ohne --mods bleibt terraria abgelehnt: der SERVER hat weiter keinen
        automatischen Weg, auch wenn seine Mods jetzt einen haben."""
        self.assertEqual(self.m.aktualisieren("terraria", live=True, mods=False), 3)
        self.assertEqual(self.a.schritte, [])

    def test_ohne_mod_block_wird_abgelehnt(self):
        """Zomboid hat einen Server-Weg, aber bewusst keinen Mod-Weg: sein Server prueft
        die WorkshopItems bei jedem Start selbst."""
        rc = self.m.aktualisieren("zomboid", live=True, mods=True)
        self.assertEqual(rc, 3)
        self.assertEqual(self.a.schritte, [])

    def test_alle_mit_mods_nimmt_nur_spiele_mit_mod_weg(self):
        sys.argv = ["spiel-aktualisieren", "--alle", "--mods"]
        self.m.main()
        self.assertEqual([x for x in self.a.schritte if "workshop" in x], ["plan:workshop:1281930"])

    def test_geaenderte_mod_staende_werden_benannt(self):
        """Ein Update, das nichts veraendert hat, darf nicht wie eines aussehen, das etwas
        getan hat. Die Ausgabe ist das einzige, was ein Mensch davon sieht."""
        # Erster Aufruf liefert den alten Stand, jeder weitere den neuen. An der Schrittzahl
        # aufgehaengt war die Attrappe vom Ablauf abhaengig und damit vom naechsten Umbau.
        staende = iter([{"2619954303": 1700000000}])
        def stand(mods):
            return next(staende, {"2619954303": 1757000000})
        self.m.mod_stand = stand
        zeilen = []
        self.m.sagen = lambda *t: zeilen.append(" ".join(str(x) for x in t))
        self.m.aktualisieren("terraria", live=True, mods=True)
        self.assertTrue(any("2619954303" in z and "->" in z for z in zeilen),
                        "der geaenderte Mod-Stand muss in der Ausgabe stehen")
